...
public class SPJFGraphingData extends JPanel {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = -7536204800731035173L;

	List<Simulation> SPJFdata;
	
    final int PAD = 35; //padding on the out side
    
   // public SPJFGraphingData(int howManyJobs){
    public SPJFGraphingData(List<RandomJobRequestGenerator> dataArrayList){
    	...
    	for(int i = 0; i < 9; i++){
    		...
    		SPJFprintRequestQueueArrayList = dataArrayList.get(i).getDataList(); //getting the random data
    		
    		for (PrintJob jobs : SPJFprintRequestQueueArrayList) {
    			jobs.resetData();
			...
			System.out.println("Speed is: " + i 
						+ " Average:" 
						+ f.format(simulationSPJF.averageTotalRunTime)
						+ " Max: "
						+ f.format(simulationSPJF.maxTurnAroundTIme));
    	
    		SPJFSimulation.add(simulationSPJF);
    	}
    	System.out.println("");
    	this.SPJFdata = SPJFSimulation;
    ...
    protected void paintComponent(Graphics g) {
       ...
       // ((Graphics2D) g).setBackground(new Color(255, 255, 255, 255));
        ...
        double xInc = (double)(w - 2*PAD)/(SPJFdata.size()-1);
        ...
        for(int i = 0; i < SPJFdata.size()-1; i++) {
           ...
           // double y1 = h - PAD - scale*FIFOdata.get(i).arrivalTime; //not sure starting poststion
            double y1 = h - PAD - scale*SPJFdata.get(i).averageTotalRunTime; //not sure starting poststion
            double x2 = PAD + (i+1)*xInc;
            //double y2 = h - PAD - scale*data[i+1];
           // double y2 = h - PAD - scale*FIFOdata.get(i+1).arrivalTime; //the height
            double y2 = h - PAD - scale*SPJFdata.get(i+1).averageTotalRunTime; //the height
        ...
        for(int i = 0; i < SPJFdata.size()-1; i++) {
            ...
            double y = h - PAD - scale*SPJFdata.get(i).averageTotalRunTime; //not sure
        ...
        for(int i = 0; i < SPJFdata.size()-1; i++) {
           ...
           // double y = h - PAD - scale*FIFOdata.get(i).turnAroundTIme; //not sure
            double y = h - PAD - scale*SPJFdata.get(i).maxTurnAroundTIme; //not sure
            //g2.fill(new Ellipse2D.Double(x-2, y-2, 4, 4));
        ...
        for(int i = 0; i < SPJFdata.size()-1; i++) {
            ...
            double y1 = h - PAD - scale*SPJFdata.get(i).maxTurnAroundTIme; //not sure
            double x2 = PAD + (i+1)*xInc;
            //double y2 = h - PAD - scale*data[i+1];
            //double y2 = h - PAD - scale*FIFOdata.get(i+1).turnAroundTIme;
            double y2 = h - PAD - scale*SPJFdata.get(i+1).maxTurnAroundTIme;
            g2.draw(new Line2D.Double(x1, y1, x2, y2));
        }
